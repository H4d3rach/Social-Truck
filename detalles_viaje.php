<?php
include ("conector.php");
session_start();
if(!isset($_SESSION['usuario']['rol'])){
    header('location: login.php');
}
if(isset($_POST['data'])){
    $id_viaje = $_POST['data'];
    $_SESSION['id_viaje']=$id_viaje;
}else{
    $id_viaje = $_SESSION['id_viaje'];

}
if(isset($_POST['inic']) && isset($_POST['fin'])){
    $inicio = $_POST['inic'];
    $fin = $_POST['fin'];
    $update = "UPDATE viaje SET hora_inicio = '$inicio' , hora_fin='$fin' WHERE id_viaje = $id_viaje";
    $upd = mysqli_query($conect,$update);
    header('location: detalles_viaje.php');
}
if(isset($_POST['precio'])){
    $precio = $_POST['precio'];
    $id_usu = $_POST['idu'];
    $cam = $_POST['camion'];
    $semi = $_POST['semi'];
    $upd = "UPDATE viaje SET precio=$precio , estatus_id=3, camion_placa='$cam',semi_no_ide=$semi,transportista_id='$id_usu'";
    $up = mysqli_query($conect,$upd);
    header('location: detalles_viaje.php');
}
$sel = "SELECT * from viaje where id_viaje=$id_viaje";
                        $resultado = mysqli_query($conect,$sel);
                        $row = $resultado -> fetch_array();
                        $recogida = $row['lugar_recogida']; 
                        $llegada = $row['lugar_destino']; 
                        $descripcion = $row['descripcion']; 
                        $precio = $row['precio']; 
                        $ntraslado = $row['nombre_traslado']; 
                        $ncontratog = $row['nompre_contratogenerico']; 
                        $ningreso = $row['nombre_ingreso']; 
                        $ncontrato = $row['nombre_contrato']; 
                        $urltraslado = $row['url_cp_traslado']; 
                        $urlcontratog = $row['url_cp_contratogenerico']; 
                        $urlingreso = $row['url_cp_ingreso']; 
                        $urlcontrato = $row['url_cp_contrato']; 
                        $fecinic = $row['fecha_inicio']; 
                        $horainic = $row['hora_inicio']; 
                        $fechafin = $row['fecha_fin']; 
                        $horafin = $row['hora_fin']; 
                        $calif = $row['calificacion']; 
                        $estatus = $row['estatus_id']; 
                        $camion = $row['camion_placa']; 
                        $semi = $row['semi_no_ide']; 
                        $transportista = $row['transportista_id']; 
                        $reptrans = $row['representante_trans_id']; 
                        $repclient = $row['representante_cliente']; 
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Representante Cliente</title>
    <link rel="stylesheet" href="css/bootstrap.min.css" />
    <link rel="stylesheet" href="css/styles.scss" />
    <link rel="stylesheet" href="css/cdn.jsdelivr.net_npm_select2@4.1.0-rc.0_dist_css_select2.min.css" />
</head>
<body>
<main class="d-flex flex-nowrap">
    <div class="d-flex flex-column flex-shrink-0 p-3 bg-body-tertiary" style="width: 250px;">
    <div class="sticky-top">
    <a href="#" class="d-flex align-items-center mb-3 mb-md-0 me-md-auto link-body-emphasis text-decoration-none">
    <img class="logo" src="imagenes/logo.png" alt="" width="45" height="35">
      <span class="fs-4">Social Truck</span>
    </a>
    <hr>
    <ul class="nav flex-column mb-auto">
      <li >
        <a href="representante_client.php" class="nav-link link-body-emphasis" aria-current="page">
          <img class="icono" src="imagenes/nueva_carga.png" alt="home">
          Publicar Trabajo
        </a>
      </li>
      <li >
        <a href="gestion_postulaciones.php" class="nav-link link-body-emphasis" aria-current="page">
          <img class="icono" src="imagenes/postulacion.png" alt="sede">
          Postulaciones
        </a>
      </li>
      <li>
        <a href="gestion_viajesaceptados.php" class="nav-link link-body-emphasis">
        <img class="icono" src="imagenes/aceptar_postulacion.png" alt="viajes">
          Viajes Aceptados
        </a>
      </li>
      <li>
        <a href="gestion_historialviajes.php" class="nav-link link-body-emphasis">
        <img class="icono" src="imagenes/historial.png" alt="viajes">
          Historial Viajes
        </a>
      </li>
      <li>
        <a href="chat.php" class="nav-link link-body-emphasis">
        <img class="icono" src="imagenes/chat.png" alt="semirremolque">
          Chat
        </a>
      </li>
      </li>
    </ul>
    <br>
    <br>
    <br>
    <br>
    <br>
    <br>
    <br>
    <br>
    <br>
    <br>
    <hr>
    <div class="dropdown">
      <a href="#" class="d-flex align-items-center link-body-emphasis text-decoration-none dropdown-toggle " data-bs-toggle="dropdown" aria-expanded="false">
        <img src="imagenes/usuario.png" alt="" width="32" height="32" class="rounded-circle me-2">
        <strong>Perfil</strong>
      </a>
      <ul class="dropdown-menu text-small shadow">
      <li><a class="dropdown-item" href="editarperfil_reptrans.php">Editar</a></li>
        <li><a class="dropdown-item" href="verperfil_client.php">Ver Perfil</a></li>
        <li><hr class="dropdown-divider"></li>
        <li><a class="dropdown-item" href="login.php?cerrar_sesion">Cerrar Sesion</a></li>
      </ul>
    </div>
</div>
  </div>
    <div class="container">
        <?php
            $rol=$_SESSION['usuario']['rol'];
             if($rol==2){
                $sel = "SELECT hora_inicio from viaje where id_viaje = $id_viaje";
                $resultado = mysqli_query($conect,$sel);
                $row = $resultado -> fetch_array();
                $inic = $row['hora_inicio'];
                if(empty($inic)){
                    echo"
                    <div class='row'>
        <div class='col-lg-6 offset-md-3 mt-4'>
            <div class='card'>
                <div class='card-header'>
                <h3 class='center'>Ajustar Horas</h3>
                </div>
                <div class='card-body'>
                  <form action='#' method='post' id='frm' class='needs-validation' novalidate >
                  <div class='row'>
                <div class='col-md-12 mb-2'>
                    <label for='inic' class='form-label'>Hora de inicio</label>
                    <input type='time' class='form-control' name='inic' id='inic' value='' >
                </div>
                </div>
                <div class='row'>
                <div class='col-md-12 mb-2'>
                    <label for='fin' class='form-label'>Hora de llegada</label>
                    <input type='time' class='form-control' name='fin' id='fin' value=''>
                </div>
                </div>
                <hr>
                <input type='submit' value='Establecer' name='establish' id='establish' class='w-100 btn btn-primary btn-lg' >
                </form>
                </div>
            </div>
        </div>
    </div>";
                }
                else{
                    $sel = "SELECT transportista_id from viaje where id_viaje = $id_viaje";
                    $resultado = mysqli_query($conect,$sel);
                    $row = $resultado -> fetch_array();
                    $inic = $row['transportista_id'];
                    if(empty($inic)){
                        echo"<h3>El representante de la empresa transportista está asignando recursos a su viaje</h3>";

                    }else{
                        //$update = "UPDATE viaje SET estatus_id=3 WHERE id_viaje=$id_viaje";
                        //$upd = mysqli_query($conect,$update);
                        
                        $sel1 = "SELECT nombre, primer_apellido FROM usuario WHERE id_usuario='$transportista'";
                        $resultado = mysqli_query($conect,$sel1);
                        $row = $resultado -> fetch_array();
                        $nombre_trans = $row['nombre'];
                        $ape_trans = $row['primer_apellido'];
                        $sel2 = "SELECT nombre, primer_apellido, empresa_razon FROM usuario WHERE id_usuario='$reptrans'";
                        $resultado = mysqli_query($conect,$sel2);
                        $row = $resultado -> fetch_array();
                        $nombre_reptrans = $row['nombre'];
                        $ape_reptrans = $row['primer_apellido'];
                        $empresa_razon = $row['empresa_razon'];
                        $sel22 = "SELECT razon_social FROM empresa WHERE razon_social='$empresa_razon'";
                        $resultado = mysqli_query($conect,$sel22);
                        $row = $resultado -> fetch_array();
                        $razon = $row['razon_social'];
                        $sel3 = "SELECT nombre, primer_apellido FROM usuario WHERE id_usuario='$repclient'";
                        $resultado = mysqli_query($conect,$sel3);
                        $row = $resultado -> fetch_array();
                        $nombre_repclient = $row['nombre'];
                        $ape_repclient = $row['primer_apellido'];
                        $sel4 = "SELECT placa,modelo FROM camion WHERE placa='$camion'";
                        $resultado = mysqli_query($conect,$sel4);
                        $row = $resultado -> fetch_array();
                        $placa = $row['placa'];
                        $modelo = $row['modelo'];
                        if($estatus==3){
                            echo"<h3>Su operador se dirige a recoger la carga</h3>";
                            echo"
                            <div class='row'>
                            <div class='col-lg-6'>
                              <div class='card'>
                                <div class='card-header'>
                                  <h3>Detalles del viaje</h3>
                                </div>
                                <div class='card-body'>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='inic' class='form-label'>Lugar Inicio</label>
                                      <input type='text' class='form-control' name='inic' id='inic' placeholder=''
                                        value='$recogida' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='dest' class='form-label'>Lugar Destino</label>
                                      <input type='text' class='form-control' name='dest' id='dest' placeholder=''
                                        value='$llegada' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='desc' class='form-label'>Descripci&oacuten</label>
                                      <textarea type='text' class='form-control' name='desc' id='desc' placeholder=''
                                        value='' readonly>$descripcion</textarea>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-6 mb-2'>
                                      <label for='finic' class='form-label'>Fecha Inicio</label>
                                      <input type='date' class='form-control' name='finic' id='finic' placeholder=''
                                        value='$fecinic' readonly>
                                    </div>
                                    <div class='col-md-6 mb-2'>
                                      <label for='hinic' class='form-label'>Hora Inicio</label>
                                      <input type='time' class='form-control' name='hinic' id='hinic' placeholder=''
                                        value='$horainic' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-6 mb-2'>
                                      <label for='ffin' class='form-label'>Fecha Fin</label>
                                      <input type='date' class='form-control' name='ffin' id='ffin' placeholder=''
                                        value='$fechafin' readonly>
                                    </div>
                                    <div class='col-md-6 mb-2'>
                                      <label for='hfin' class='form-label'>Hora Fin</label>
                                      <input type='time' class='form-control' name='hfin' id='hfin' placeholder=''
                                        value='$horafin' readonly>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div class='col-lg-6'>
                              <div class='card'>
                                <div class='card-header'>
                                  <h3>Configuraci&oacuten</h3>
                                </div>
                                <div class='card-body'>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='etrans' class='form-label'>Empresa de transporte</label>
                                      <input type='text' class='form-control' name='etrans' id='etrans' placeholder=''
                                        value='$razon' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='rtrans' class='form-label'>Representante</label>
                                      <input type='text' class='form-control' name='rtrans' id='rtrans' placeholder=''
                                        value='$nombre_reptrans $ape_reptrans' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='oper' class='form-label'>Operador</label>
                                      <input type='text' class='form-control' name='oper' id='oper' placeholder=''
                                        value='$nombre_trans $ape_trans' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-6 mb-2'>
                                      <label for='placas' class='form-label'>Placas</label>
                                      <input type='text' class='form-control' name='placas' id='placas' placeholder=''
                                        value='$placa' readonly>
                                    </div>
                                    <div class='col-md-6 mb-2'>
                                      <label for='camion' class='form-label'>Cami&oacuten</label>
                                      <input type='text' class='form-control' name='camion' id='camion' placeholder=''
                                        value='$modelo' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-6 mb-2'>
                                      <label for='noid' class='form-label'>No. Identificaci&oacuten Remolque</label>
                                      <input type='text' class='form-control' name='noid' id='noid' placeholder=''
                                        value='$semi' readonly>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            
                          </div>
                            ";
                        }else if($estatus==4){
                            echo"<h3>Su operador ya embarc&oacute y se dirige a su destino</h3>";
                            echo"
                            <div class='row'>
                            <div class='col-lg-6'>
                              <div class='card'>
                                <div class='card-header'>
                                  <h3>Detalles del viaje</h3>
                                </div>
                                <div class='card-body'>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='inic' class='form-label'>Lugar Inicio</label>
                                      <input type='text' class='form-control' name='inic' id='inic' placeholder=''
                                        value='$recogida' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='dest' class='form-label'>Lugar Destino</label>
                                      <input type='text' class='form-control' name='dest' id='dest' placeholder=''
                                        value='$llegada' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='desc' class='form-label'>Descripci&oacuten</label>
                                      <textarea type='text' class='form-control' name='desc' id='desc' placeholder=''
                                        value='' readonly>$descripcion</textarea>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-6 mb-2'>
                                      <label for='finic' class='form-label'>Fecha Inicio</label>
                                      <input type='date' class='form-control' name='finic' id='finic' placeholder=''
                                        value='$fecinic' readonly>
                                    </div>
                                    <div class='col-md-6 mb-2'>
                                      <label for='hinic' class='form-label'>Hora Inicio</label>
                                      <input type='time' class='form-control' name='hinic' id='hinic' placeholder=''
                                        value='$horainic' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-6 mb-2'>
                                      <label for='ffin' class='form-label'>Fecha Fin</label>
                                      <input type='date' class='form-control' name='ffin' id='ffin' placeholder=''
                                        value='$fechafin' readonly>
                                    </div>
                                    <div class='col-md-6 mb-2'>
                                      <label for='hfin' class='form-label'>Hora Fin</label>
                                      <input type='time' class='form-control' name='hfin' id='hfin' placeholder=''
                                        value='$horafin' readonly>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div class='col-lg-6'>
                              <div class='card'>
                                <div class='card-header'>
                                  <h3>Configuraci&oacuten</h3>
                                </div>
                                <div class='card-body'>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='etrans' class='form-label'>Empresa de transporte</label>
                                      <input type='text' class='form-control' name='etrans' id='etrans' placeholder=''
                                        value='$razon' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='rtrans' class='form-label'>Representante</label>
                                      <input type='text' class='form-control' name='rtrans' id='rtrans' placeholder=''
                                        value='$nombre_reptrans $ape_reptrans' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='oper' class='form-label'>Operador</label>
                                      <input type='text' class='form-control' name='oper' id='oper' placeholder=''
                                        value='$nombre_trans $ape_trans' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-6 mb-2'>
                                      <label for='placas' class='form-label'>Placas</label>
                                      <input type='text' class='form-control' name='placas' id='placas' placeholder=''
                                        value='$placa' readonly>
                                    </div>
                                    <div class='col-md-6 mb-2'>
                                      <label for='camion' class='form-label'>Cami&oacuten</label>
                                      <input type='text' class='form-control' name='camion' id='camion' placeholder=''
                                        value='$modelo' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-6 mb-2'>
                                      <label for='noid' class='form-label'>No. Identificaci&oacuten Remolque</label>
                                      <input type='text' class='form-control' name='noid' id='noid' placeholder=''
                                        value='$semi' readonly>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            
                          </div>
                            ";
                        }else if($estatus==5){
                            echo"
                            <script>
                        const formatter = new Intl.NumberFormat('en-US', {
                            style: 'currency',
                            currency: 'USD',
                        
                        });

                           var resumen = {
                              idViaje: '".$id_viaje."',
                              lugarRecogida : '".$recogida."',
                              destino: '".$llegada."',
                              transportista : '".$nombre_trans."',
                              empresaTrans : '".$razon."',
                              precioTotal : formatter.format('".$precio."'),
                              tarjeta: ''
                           };
                       
                           let params = {
                           'method': 'POST',
                           'headers': {
                              'Content-Type': 'application/json; charset=utf-8'
                           },
                           'body': JSON.stringify(resumen) 
                          }
                      
                           function getVal(){
                            resumen.tarjeta = document.getElementById('numTarjeta').value
                            //console.log(resumen.tarjeta);
                            fetch('pago2.php', params)
                        
                          }

                        </script>
                            ";
                            echo"<div class='position-absolute top-0 start-50 translate-middle-x'>
                            <h1>
                            Proceder al pago
                            </h1>
                            <h3>Su operador ya desembarc&oacute</h3>
                            </div>";
                            echo"<br><br>
                                <div class='row'>
                                  <div class='col-7 px-2' id='columna-izq'>
                                    <div  class='position-relative top-20 start-10 w-100 p-5'id='detalles-de-envio'>
                                    <table>
                                      <tr>
                                        <th>ID del viaje</th>
                                        <th id = 'idViaje'>
                                          <script>
                                            document.getElementById('idViaje').innerHTML = resumen.idViaje;
                                          </script>
                                        </th>
                                      </tr>
                                      <tr>
                                        <td>Lugar de Recogida</td>
                                        <td id = 'lugarRecogida'>
                                          <script>
                                            document.getElementById('lugarRecogida').innerHTML = resumen.lugarRecogida;
                                          </script></td>
                                      </tr>
                                      <tr>
                                        <td>Lugar de Destino</td>
                                        <td id = 'destino'><script>
                                            document.getElementById('destino').innerHTML = resumen.destino;
                                          </script></td>
                                      </tr>
                                      <tr>
                                        <td>Transportista</td>
                                        <td id= 'transportista'><script>
                                            document.getElementById('transportista').innerHTML = resumen.transportista;
                                          </script></td>
                                      </tr>
                                      <tr>
                                        <td>Empresa</td>
                                        <td id = 'empresaTrans'><script>
                                            document.getElementById('empresaTrans').innerHTML = resumen.empresaTrans;
                                          </script></td>
                                      </tr>
                                    </table>
                                    </div>
                                    <div id='metodo-de-pago' class='mx-auto p-5'>
                                      <div class='popover p-1' >
                                        <header class='popover-header'><h4>Agregar una tarjeta de crédito o débito</h4></header>
                                        <div>
                                            <div class='form-outline'>
                                              <input type='text' id='numTarjeta' class='form-control form-control-lg' siez='17'
                                                placeholder='1234 5678 9012 3457' minlength='19' maxlength='19' />
                                              <label class='form-label' for='numTarjeta' > Número de tarjeta</label>
                                            </div>
                                            <div class='d-flex justify-content-between align-items-center mb-4'>
                                              <div class='form-outline'>
                                                <input type='text' id='typeName' class='form-control form-control-lg' siez='17'
                                                  placeholder='Nombre en la tarjeta' />
                                                <label class='form-label' for='typeName'>Nombre en la tarjeta</label>
                                              </div>
                                            </div>
                                          <div class='d-flex justify-content-between align-items-center pb-2'>
                                            <div class='form-outline'>
                                              <input type='text' id='typeExp' class='form-control form-control-lg' placeholder='MM/YYYY'
                                                size='7' id='exp' minlength='7' maxlength='7' />
                                              <label class='form-label' for='typeExp'>Fecha de expiración</label>
                                            </div>
                                            <div class='form-outline'>
                                              <input type='password' id='typeText2' class='form-control form-control-lg'
                                                placeholder='&#9679;&#9679;&#9679;' size='1' minlength='3' maxlength='3' />
                                              <label class='form-label' for='typeText2'>CVV</label>
                                            </div>
                                        </div>  
                                      </div>
                                    </div>
                                    <div class='position-relative p-2' id='boton_proceder'>
                                    <form action='pago2.php' method='POST'><input type='hidden' name='data' id='data' value='".$id_viaje."'><button type='submit' class='btn btn-primary'>Proceder</button></form>
                                    </div>
                                  </div>
                                  </div>
                                  <br>
                                  <br>
                                  <div class='col-md-auto mt-5' id='column-der'>
                                  <div class='p-2 bd-highlight' style='background-color: #f8f9fa;'>
                                        <div class='p-2 me-3'>
                                          <h4>Resumen de la orden</h4>
                                        </div>
                                        <div class='p-2 d-flex'>
                                          <div class='col-8'>Precio contratado</div>
                                          <div class='ms-auto' id='precioTotal'>
                                            <script>
                                              document.getElementById('precioTotal').innerHTML = resumen.precioTotal;
                                            </script></div>
                                        </div>
                                        <div class='border-top px-2 mx-2'></div>
                                        <div class='p-2 d-flex pt-3'>
                                          <div class='col-8'><b>Total</b></div>
                                            <div class='ms-auto'><b class='text-success' id='total'>
                                              <script>
                                                document.getElementById('total').innerHTML = resumen.precioTotal;
                                              </script>
                                              </b>
                                            </div>
                                          </div>
                                        </div>
                                  </div>
                                </div>
                                ";
                        }else if($estatus==6){
                            echo"<h3>Viaje completado y pagado</h3>";
                            echo"
                            <div class='row'>
                            <div class='col-lg-6'>
                              <div class='card'>
                                <div class='card-header'>
                                  <h3>Detalles del viaje</h3>
                                </div>
                                <div class='card-body'>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='inic' class='form-label'>Lugar Inicio</label>
                                      <input type='text' class='form-control' name='inic' id='inic' placeholder=''
                                        value='$recogida' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='dest' class='form-label'>Lugar Destino</label>
                                      <input type='text' class='form-control' name='dest' id='dest' placeholder=''
                                        value='$llegada' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='desc' class='form-label'>Descripci&oacuten</label>
                                      <textarea type='text' class='form-control' name='desc' id='desc' placeholder=''
                                        value='' readonly>$descripcion</textarea>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-6 mb-2'>
                                      <label for='finic' class='form-label'>Fecha Inicio</label>
                                      <input type='date' class='form-control' name='finic' id='finic' placeholder=''
                                        value='$fecinic' readonly>
                                    </div>
                                    <div class='col-md-6 mb-2'>
                                      <label for='hinic' class='form-label'>Hora Inicio</label>
                                      <input type='time' class='form-control' name='hinic' id='hinic' placeholder=''
                                        value='$horainic' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-6 mb-2'>
                                      <label for='ffin' class='form-label'>Fecha Fin</label>
                                      <input type='date' class='form-control' name='ffin' id='ffin' placeholder=''
                                        value='$fechafin' readonly>
                                    </div>
                                    <div class='col-md-6 mb-2'>
                                      <label for='hfin' class='form-label'>Hora Fin</label>
                                      <input type='time' class='form-control' name='hfin' id='hfin' placeholder=''
                                        value='$horafin' readonly>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div class='col-lg-6'>
                              <div class='card'>
                                <div class='card-header'>
                                  <h3>Configuraci&oacuten</h3>
                                </div>
                                <div class='card-body'>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='etrans' class='form-label'>Empresa de transporte</label>
                                      <input type='text' class='form-control' name='etrans' id='etrans' placeholder=''
                                        value='$razon' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='rtrans' class='form-label'>Representante</label>
                                      <input type='text' class='form-control' name='rtrans' id='rtrans' placeholder=''
                                        value='$nombre_reptrans $ape_reptrans' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-12 mb-2'>
                                      <label for='oper' class='form-label'>Operador</label>
                                      <input type='text' class='form-control' name='oper' id='oper' placeholder=''
                                        value='$nombre_trans $ape_trans' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-6 mb-2'>
                                      <label for='placas' class='form-label'>Placas</label>
                                      <input type='text' class='form-control' name='placas' id='placas' placeholder=''
                                        value='$placa' readonly>
                                    </div>
                                    <div class='col-md-6 mb-2'>
                                      <label for='camion' class='form-label'>Cami&oacuten</label>
                                      <input type='text' class='form-control' name='camion' id='camion' placeholder=''
                                        value='$modelo' readonly>
                                    </div>
                                  </div>
                                  <div class='row'>
                                    <div class='col-md-6 mb-2'>
                                      <label for='noid' class='form-label'>No. Identificaci&oacuten Remolque</label>
                                      <input type='text' class='form-control' name='noid' id='noid' placeholder=''
                                        value='$semi' readonly>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            
                          </div>
                            ";
                        }
                    }
            }

            }
        ?>
</div>
</main>
<script src="js/cdn.jsdelivr.net_npm_select2@4.1.0-rc.0_dist_js_select2.min.js"></script>
  <script src="js/bootstrap.bundle.min.js"></script>
</body>
</html>